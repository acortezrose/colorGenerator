...
function App() {
	...
	function generateCircleSamples(baseColor, N, harmony) {
		...
		if (harmony === "Equidistant") {
			for (let i = 0; i < N; i++) {
				...
				const lightness1 = baseColor.l + lightnessShiftFactor / 1.5;
				const lightness2 = baseColor.l + lightnessShiftFactor;
				samples.push({
					l,
					c,
					h: newHue,
					css: `oklch(${l} ${c} ${newHue})`,
		...
		// Generate N circle samples by varying lightness while keeping hue and chroma constant
			...
			for (let i = 0; i < N; i++) {
				const newLightness = (l - (i * 0.6) / N) % 1;
				const shiftHue1 = (h - hueShiftFactor) % 360;
				const lightness1 = newLightness + lightnessShiftFactor / 1.5;
				const lightness2 = newLightness + lightnessShiftFactor;
				samples.push({
					l: newLightness,
					c,
					h,
	...
	return (
		<div className="grid">
			<div>
				<h1>Avatar Color Palette Generator</h1>
				...
				<AnimatePresence>
					{swatchData.circleSamples.map((color, i) => (
						<motion.div key={i} className="sample">
							...
							>
								<title>Avatar background {i}</title>
